##### Performance in identification of risk SNPs if treat the effects of all covariates as fixed effects #####
# Vary alpha=0.2, 0.4, 0.6 to get 
# Figures S9 in Supplementary Document
...
for (i in 1:rep){
  ...
  fit1 <- LSMM(data$Pvalue, cbind(data$Z, data$A), NULL)
  ...
  result[i, 5:8] <- as.numeric(performance(data$gamma, assoc.SNP1$gamma, 1-fit1$pi1))
}

result <- as.data.frame(result)
names(result) <- c("FDR.LSMM", "power.LSMM", "AUC.LSMM", "pAUC.LSMM", 
                   "FDR.LSMM.treat.as.fixed", "power.LSMM.treat.as.fixed", 
                   "AUC.LSMM.treat.as.fixed", "pAUC.LSMM.treat.as.fixed")
